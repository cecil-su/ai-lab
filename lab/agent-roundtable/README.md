...
- **稳定前缀设计(同项目跨话题缓存命中)**:把 charter 拆为"稳定段(项目基线+代码注入)+ 可变段(标题/议题/参与者)",使同一项目的多个话题共享 prompt 前缀 → 跨话题命中各家前缀缓存(代码段按 ~0.1x 计)。当前实现 charter 从标题即分叉,跨话题不命中。改动范围:charter 组装 + 实测跨话题命中率。
- **语义抽检**:verify 只验引用完整性;结论支撑度需模型/人抽检,做成轻量工具。
- **跨话题 seq 契约**:把"seq 是 transcript 内局部序"写死进 spec(防未来全局唯一性踩坑)。
- **熔断阈值复核**:终审风险项,全场景失败烧满 3 轮才 paused,阈值是否偏大需真机数据。