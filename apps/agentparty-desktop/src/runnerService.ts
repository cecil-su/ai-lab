...
export class TauriRunnerService implements RunnerService {
  ...
  runRunner(agentConfig: LocalAgentConfig, context: RunnerContext): Promise<RunnerResult> {
    if (agentConfig.runnerKind === "codex") return this.runCodexRunner(agentConfig, context);
    if (agentConfig.runnerKind === "custom-command") return this.runCustomCommandRunner(agentConfig, context);
    return this.runFakeRunner(agentConfig, context);
  }
  ...
  runCustomCommandRunner(agentConfig: LocalAgentConfig, context: RunnerContext): Promise<RunnerResult> {
    return this.invoke("run_custom_command_runner", { agentConfig, context });
  }
...
export class MemoryRunnerService implements RunnerService {
  ...
  async runCustomCommandRunner(agentConfig: LocalAgentConfig, context: RunnerContext): Promise<RunnerResult> {
    const result: RunnerResult = {
      status: agentConfig.customCommand ? "done" : "blocked",
      draftReply: agentConfig.customCommand ? `Custom command ${agentConfig.name} saw: ${context.triggeringMessage.body}` : "",
      stdout: agentConfig.customCommand ? `custom command handled ${context.triggeringMessage.id}` : "",
      stderr: agentConfig.customCommand ? "" : "Custom command is required",
      exitCode: agentConfig.customCommand ? 0 : 1,
      contextFilePath: `${agentConfig.workdir.replace(/\\+$/, "")}\\runner-context-${context.triggeringMessage.id}.json`,
    };
    this.logs.push({
      ...result,
